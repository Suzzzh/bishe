...
<script>
import { listFail, getFail, delFail, addFail, updateFail } from "@/api/bishe/fail";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加不及格学生名单";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const courseId = row.courseId || this.ids
      getFail(courseId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改不及格学生名单";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除不及格学生名单编号为"' + courseIds + '"的数据项？').then(function() {
        return delFail(courseIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('bishe/fail/export', {
        ...this.queryParams
